...
from src.mistral_parser import parse_with_mistral
...
parser_choice = st.selectbox(
    "Select Parser",
    ["Unstructured", "Qwen Vision","Mistral OCR"]
)
...
if uploaded_file:
    ...
    with st.spinner(f"🔄 Parsing with {parser_choice}..."):
        ...
        elif parser_choice == "Mistral OCR":
            pages = convert_from_path(file_path)
            full_output = ""
            for i, page in enumerate(pages):
                st.write(f"Processing Page {i+1}...")
                page_markdown = parse_with_mistral(page)
                full_output += f"\n\n## Page {i+1}\n\n{page_markdown}"
